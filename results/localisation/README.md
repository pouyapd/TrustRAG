...
`scripts/localisation_report.py` from the per-question rows in this directory and
pasted unedited; the prose quotes those tables. No number here is carried over from an
earlier draft.
...
- **Chunk size raises raw localisation only by shrinking the candidate set.** MiniLM hit@1
  goes 0.335 → 0.376 → 0.431 from 128 to 512 tokens while the lift over chance falls
